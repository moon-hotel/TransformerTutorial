...
class MyTransformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask=None, tgt_mask=None,
        ...
        :return: [tgt_len, batch_size, num_heads * head_dim] <==> [tgt_len,batch_size,embed_dim]
        """
        memory = self.encoder(src, mask=src_mask, src_key_padding_mask=src_key_padding_mask)
        # [src_len, batch_size, num_heads * head_dim] <==> [src_len,batch_size,embed_dim]
        output = self.decoder(tgt=tgt, memory=memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
                              tgt_key_padding_mask=tgt_key_padding_mask,
                              memory_key_padding_mask=memory_key_padding_mask)
        return output  # [tgt_len, batch_size, num_heads * head_dim] <==> [tgt_len, batch_size, embed_dim]
...
class MyTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # src2: [src_len, batch_size, num_heads*head_dim] num_heads*head_dim = embed_dim
        src = src + self.dropout1(src2)  # 残差连接
        src = self.norm1(src)  # [src_len,batch_size,num_heads*head_dim]

        src2 = self.activation(self.linear1(src))  # [src_len,batch_size,dim_feedforward]
        src2 = self.linear2(self.dropout(src2))  # [src_len,batch_size,num_heads*head_dim]
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        return src  # [src_len, batch_size, num_heads * head_dim] <==> [src_len,batch_size,embed_dim]
...
class MyTransformerEncoder(nn.Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None):
        ...
        :return:# [src_len, batch_size, num_heads * head_dim] <==> [src_len,batch_size,embed_dim]
        """
        output = src
        ...
        return output  # [src_len, batch_size, num_heads * head_dim] <==> [src_len,batch_size,embed_dim]
...
class MyTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
        ...
        tgt = self.norm3(tgt)
        return tgt  # [tgt_len, batch_size, num_heads * head_dim] <==> [tgt_len,batch_size,embed_dim]
